...
def getnetworkclients(apikey, networkid, timespan=86400, perpage=3, suppressprint=False):
    calltype = 'Device Clients'
    geturl = '{0}/networks/{1}/clients?timespan={2}&perPage={3}'.format(str(base_url), str(networkid), timespan,
                                                                        perpage)
    headers = {
        'x-cisco-meraki-api-key': format(str(apikey)),
        'Content-Type': 'application/json'
...
def getclientpolicy(apikey, networkid, clientmac, timestamp=86400, suppressprint=False):
    ...
    if timestamp > 2592000:
        timestamp = 2592000
    geturl = '{0}/networks/{1}/clients/{2}/policy?timespan={3}'.format(str(base_url), str(networkid), str(clientmac),
                                                                       str(timestamp))
    headers = {
        'x-cisco-meraki-api-key': format(str(apikey)),
        'Content-Type': 'application/json'
...
def updateclientpolicy(apikey, networkid, clientmac, policy, policyid=None, suppressprint=False):
    calltype = 'Device Clients'
    puturl = '{0}/networks/{1}/clients/{2}/policy?timespan=2592000'.format(str(base_url), str(networkid),
                                                                           str(clientmac))
    headers = {
        'x-cisco-meraki-api-key': format(str(apikey)),
        'Content-Type': 'application/json'
...
def getclientsplash(apikey, networkid, clientmac, suppressprint=False):
    calltype = 'Device Clients'
    geturl = '{0}/networks/{1}/clients/{2}/splashAuthorizationStatus'.format(str(base_url), str(networkid),
                                                                             str(clientmac))
    headers = {
        'x-cisco-meraki-api-key': format(str(apikey)),
        'Content-Type': 'application/json'
...
def updateclientsplash(apikey, networkid, clientmac, ssid_authorization, suppressprint=False):
    ...
    calltype = 'Device Clients'
    puturl = '{0}/networks/{1}/clients/{2}/splashAuthorizationStatus'.format(str(base_url), str(networkid),
                                                                             str(clientmac))
    headers = {
        'x-cisco-meraki-api-key': format(str(apikey)),
        'Content-Type': 'application/json'
...
def getclientevents(apikey, networkid, clientid, unixtime, suppressprint=False):
    calltype = 'Client Detail'
    geturl = '{0}/networks/{1}/clients/{2}/events?startingAfter={3}?'.format(str(base_url), str(networkid),
                                                                             str(clientid), str(unixtime))
    headers = {
        'x-cisco-meraki-api-key': format(str(apikey)),
        'Content-Type': 'application/json'
    }
...
def getclienttraffichistory(apikey, networkid, clientid, suppressprint=False):
    ...
    geturl = '{0}/networks/{1}/clients/{2}/trafficHistory?perPage=50'.format(str(base_url), str(networkid),
                                                                             str(clientid))
    headers = {
        'x-cisco-meraki-api-key': format(str(apikey)),
        'Content-Type': 'application/json'
...
def getOrganizationUplinksLossAndLatency(apikey, organizationid, suppressprint=False):
    ...
    geturl = '{0}/organizations/{1}/uplinksLossAndLatency'.format(str(base_url), str(organizationid))
    headers = {
        'x-cisco-meraki-api-key': format(str(apikey)),
        'Content-Type': 'application/json'
    }
...
def updatebluetooth(apikey, networkid, scanning=False, advertising=False, uuid=None, nonunique=False, major=None,
                    minor=None, suppressprint=False):
    calltype = 'Network Detail'
...
def getsmdevices(apikey, networkid, fields=None, wifimacs=None, serials=None, ids=None, scope=None, token=None,
    ...
    possiblefields = ['ip', 'systemType', 'availableDeviceCapacity', 'kioskAppName', 'biosVersion', 'lastConnected',
                      'missingAppsCount', 'userSuppliedAddress', 'location', 'lastUser', 'publicIp', 'phoneNumber',
                      'diskInfoJson', 'deviceCapacity', 'isManaged', 'hadMdm', 'isSupervised', 'meid', 'imei', 'iccid',
                      'simCarrierNetwork', 'cellularDataUsed', 'isHotspotEnabled', 'createdAt', 'batteryEstCharge',
                      'quarantined', 'avName', 'avRunning', 'asName', 'fwName', 'isRooted', 'loginRequired',
                      'screenLockEnabled', 'screenLockDelay', 'autoLoginDisabled', 'hasMdm', 'hasDesktopAgent',
    ...
    if token != None:
        ...
        if scope != None:
            if scope.split(',')[0] not in ('all', 'none', 'withAny', 'withAll', 'withoutAny', 'withoutAll'):
                raise ValueError(
                    'Scope (one of all, none, withAny, withAll, withoutAny, or withoutAll) must be specified')
            geturl += 'scope=' + scope
...
def updatesmtags(apikey, networkid, tags, action, wifimacs=None, ids=None, serials=None, scope=None,
    ...
    if scope != None:
        if scope.split(',')[0] not in ('all', 'none', 'withAny', 'withAll', 'withoutAny', 'withoutAll'):
            raise ValueError(
                'Parameter scope (one of all, none, withAny, withAll, withoutAny, or withoutAll) must be specified')
        putdata['scope'] = scope
...
def updatessid(apikey, networkid, ssidnum, name, enabled, authmode, encryptionmode, ipassignmentmode, psk=None,
               vlan=None, suppressprint=False):
    calltype = 'SSID'
...
def updateswitchport(apikey, serialnum, portnum, name=None, tags=None, enabled=None, porttype=None, vlan=None,
                     ...
                     accesspolicynum=None, suppressprint=False):
    calltype = 'Switch Port'
